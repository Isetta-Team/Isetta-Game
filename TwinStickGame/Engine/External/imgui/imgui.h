...
// Read 'Programmer guide' in imgui.cpp for notes on how to setup ImGui in your
// codebase. Get latest version at https://github.com/ocornut/imgui

#pragma once

// Configuration file (edit imconfig.h or define IMGUI_USER_CONFIG to set your
// own filename)
...
// Version
// (Integer encoded as XYYZZ for use in #if preprocessor conditionals. Work in
// progress versions typically starts at XYY00 then bounced up to XYY01 when
...
#define IMGUI_CHECKVERSION()                                                \
  ImGui::DebugCheckVersionAndDataLayout(IMGUI_VERSION, sizeof(ImGuiIO),     \
                                        sizeof(ImGuiStyle), sizeof(ImVec2), \
...
struct ImDrawChannel;  // Temporary storage for outputting drawing commands out
                       // of order, used by ImDrawList::ChannelsSplit()
struct ImDrawCmd;      // A single draw command within a parent ImDrawList
                       // (generally maps to 1 GPU draw call)
struct ImDrawData;     // All draw command lists required to render the frame
struct ImDrawList;     // A single draw command list (generally one per window,
                       // conceptually you may see this as a dynamic "mesh"
                       // builder)
struct ImDrawListSharedData;  // Data shared among multiple draw lists
                              // (typically owned by parent ImGui context, but
                              // you may create one yourself)
struct ImDrawVert;  // A single vertex (20 bytes by default, override layout
                    // with IMGUI_OVERRIDE_DRAWVERT_STRUCT_LAYOUT)
struct ImFont;  // Runtime data for a single font within a parent ImFontAtlas
struct ImFontAtlas;   // Runtime data for multiple fonts, bake multiple fonts
...
#ifndef ImTextureID
typedef void*
    ImTextureID;  // User data to identify a texture (this is whatever to you
                  // want it to be! read the FAQ about ImTextureID in imgui.cpp)
#endif
struct ImGuiContext;  // ImGui context (opaque)
struct ImGuiIO;       // Main configuration and I/O between your application and
                      // ImGui
...
struct ImGuiOnceUponAFrame;  // Helper for running a block of code not more than
                             // once a frame, used by IMGUI_ONCE_UPON_A_FRAME
                             // macro
...
struct ImGuiTextFilter;        // Helper to parse and apply text filters (e.g.
                               // "aaaaa[,bbbb][,ccccc]")
struct ImGuiTextBuffer;        // Helper to hold and append into a text buffer
                               // (~string builder)

// Typedefs and Enums/Flags (declared as int for compatibility with old C++, to
// allow using as flags and to not pollute the top of this file) Use your
// programming IDE "Go to definition" facility on the names of the center
// columns to find the actual flags/enum lists.
...
typedef int
    ImGuiCond;  // -> enum ImGuiCond_            // Enum: A condition for Set*()
typedef int ImGuiDataType;  // -> enum ImGuiDataType_        // Enum: A primary
                            // data type
typedef int
    ImGuiDir;  // -> enum ImGuiDir_             // Enum: A cardinal direction
typedef int ImGuiKey;       // -> enum ImGuiKey_             // Enum: A key
                            // identifier (ImGui-side enum)
typedef int ImGuiNavInput;  // -> enum ImGuiNavInput_        // Enum: An input
...
typedef int ImGuiStyleVar;  // -> enum ImGuiStyleVar_        // Enum: A variable
                            // identifier for styling
typedef int ImDrawCornerFlags;  // -> enum ImDrawCornerFlags_    // Flags: for
                                // ImDrawList::AddRect*() etc.
typedef int
    ImDrawListFlags;  // -> enum ImDrawListFlags_      // Flags: for ImDrawList
...
typedef int ImGuiColumnsFlags;    // -> enum ImGuiColumnsFlags_    // Flags: for
                                  // Columns(), BeginColumns()
typedef int ImGuiConfigFlags;     // -> enum ImGuiConfigFlags_     // Flags: for
                                  // io.ConfigFlags
typedef int ImGuiComboFlags;      // -> enum ImGuiComboFlags_      // Flags: for
                                  // BeginCombo()
typedef int ImGuiDragDropFlags;   // -> enum ImGuiDragDropFlags_   // Flags: for
                                  // *DragDrop*()
...
typedef int ImGuiInputTextFlags;  // -> enum ImGuiInputTextFlags_  // Flags: for
                                  // InputText*()
typedef int ImGuiSelectableFlags;  // -> enum ImGuiSelectableFlags_ // Flags:
                                   // for Selectable()
typedef int ImGuiTreeNodeFlags;  // -> enum ImGuiTreeNodeFlags_   // Flags: for
                                 // TreeNode*(),CollapsingHeader()
typedef int
    ImGuiWindowFlags;  // -> enum ImGuiWindowFlags_     // Flags: for Begin*()
...
typedef signed __int64
    ImS64;  // 64-bit signed integer (pre and post C++11 with Visual Studio)
typedef unsigned __int64
    ImU64;  // 64-bit unsigned integer (pre and post C++11 with Visual Studio)
...
// (In a namespace so you can add extra functions in your own separate file.
// Please don't modify imgui.cpp/.h!)
namespace ImGui {
...
// contexts. All those functions are not reliant on the current context.
IMGUI_API ImGuiContext* CreateContext(ImFontAtlas* shared_font_atlas = NULL);
IMGUI_API void DestroyContext(
    ImGuiContext* ctx = NULL);  // NULL = destroy current context
...
IMGUI_API void
NewFrame();  // start a new ImGui frame, you can submit any command from this
             // point until Render()/EndFrame().
IMGUI_API void
EndFrame();  // ends the ImGui frame. automatically called by Render(), you
             // likely don't need to call that yourself directly. If you don't
             // need to render data (skipping rendering) you may call EndFrame()
             // but you'll have wasted CPU already! If you don't need to render,
             // better to not create any imgui windows and not call NewFrame()
             // at all!
IMGUI_API void
Render();  // ends the ImGui frame, finalize the draw data. (Obsolete:
           // optionally call io.RenderDrawListsFn if set. Nowadays, prefer
...
GetDrawData();  // valid after Render() and until the next call to NewFrame().
                // this is what you have to render. (Obsolete: this used to be
                // passed to your io.RenderDrawListsFn() function.)

// Demo, Debug, Information
IMGUI_API void ShowDemoWindow(
    bool* p_open =
        NULL);  // create demo/test window (previously called ShowTestWindow).
...
IMGUI_API void ShowMetricsWindow(
    bool* p_open = NULL);  // create metrics window. display ImGui internals:
                           // draw commands (with individual draw calls and
                           // vertices), window list, basic internal state, etc.
...
IMGUI_API void ShowFontSelector(
    const char* label);  // add font selector block (not a window), essentially
                         // a combo listing the loaded fonts.
IMGUI_API void
...
GetVersion();  // get the compiled version string e.g. "1.23"

// Styles
IMGUI_API void StyleColorsDark(
    ImGuiStyle* dst = NULL);  // new, recommended style (default)
IMGUI_API void StyleColorsClassic(
    ImGuiStyle* dst = NULL);  // classic imgui style
IMGUI_API void StyleColorsLight(
    ImGuiStyle* dst =
        NULL);  // best used with borders and a custom, thicker font

// Windows
// (Begin = push window to the stack and start appending to it. End = pop window
// from the stack. You may append multiple times to the same window during the
// same frame) Begin()/BeginChild() return false to indicate the window being
// collapsed or fully clipped, so you may early out and omit submitting anything
// to the window. You need to always call a matching End()/EndChild() for a
// Begin()/BeginChild() call, regardless of its return value (this is due to
// legacy reason and is inconsistent with BeginMenu/EndMenu, BeginPopup/EndPopup
// and other functions where the End call should only be called if the
// corresponding Begin function returned true.) Passing 'bool* p_open != NULL'
// shows a close widget in the upper-right corner of the window, which when
// clicking will set the boolean to false. Use child windows to introduce
// independent scrolling/clipping regions within a host window. Child windows
// can embed their own child.
IMGUI_API bool Begin(const char* name, bool* p_open = NULL,
                     ImGuiWindowFlags flags = 0);
IMGUI_API void End();
IMGUI_API bool BeginChild(
    const char* str_id, const ImVec2& size = ImVec2(0, 0), bool border = false,
    ImGuiWindowFlags flags =
        0);  // Begin a scrolling region. size==0.0f: use remaining window size,
             // size<0.0f: use remaining window size minus abs(size). size>0.0f:
             // fixed size. each axis can use a different mode, e.g.
             // ImVec2(0,400).
...
IMGUI_API bool IsWindowHovered(
    ImGuiHoveredFlags flags =
        0);  // is current window hovered (and typically: not blocked by a
             ...
             // check whether your mouse should be dispatched to imgui or to
             // your app, you should use the 'io.WantCaptureMouse' boolean for
             // that! Please read the FAQ!
IMGUI_API ImDrawList*
GetWindowDrawList();  // get draw list associated to the window, to append your
                      // own drawing primitives
IMGUI_API ImVec2
GetWindowPos();  // get current window position in screen space (useful if you
                 // want to do your own drawing via the DrawList API)
...
IMGUI_API ImVec2
GetWindowContentRegionMin();  // content boundaries min (roughly (0,0)-Scroll),
                              // in window coordinates
IMGUI_API ImVec2
GetWindowContentRegionMax();  // content boundaries max (roughly
                              // (0,0)+Size-Scroll) where Size can be override
                              // with SetNextWindowContentSize(), in window
                              // coordinates
...
IMGUI_API void SetNextWindowContentSize(
    const ImVec2& size);  // set next window content size (~ enforce the range
                          // of scrollbars). not including window decorations
                          // (title bar, menu bar, etc.). set an axis to 0.0f to
                          // leave it automatic. call before Begin()
IMGUI_API void SetNextWindowCollapsed(
    bool collapsed,
    ImGuiCond cond =
        0);  // set next window collapsed state. call before Begin()
IMGUI_API void SetNextWindowFocus();  // set next window to be focused /
                                      // front-most. call before Begin()
IMGUI_API void SetNextWindowBgAlpha(
    float alpha);  // set next window background color alpha. helper to easily
                   // modify ImGuiCol_WindowBg/ChildBg/PopupBg.
IMGUI_API void SetWindowPos(
    const ImVec2& pos,
    ImGuiCond cond =
        0);  // (not recommended) set current window position - call within
             // Begin()/End(). prefer using SetNextWindowPos(), as this may
...
IMGUI_API void SetWindowFontScale(
    float scale);  // set font scale. Adjust IO.FontGlobalScale if you want to
                   // scale all windows
...
IMGUI_API void SetWindowFocus(
    const char* name);  // set named window to be focused / front-most. use NULL
                        // to remove focus.
...
IMGUI_API float GetScrollMaxX();  // get maximum scrolling amount ~~
                                  // ContentSize.X - WindowSize.X
IMGUI_API float GetScrollMaxY();  // get maximum scrolling amount ~~
                                  // ContentSize.Y - WindowSize.Y
IMGUI_API void SetScrollX(
    float scroll_x);  // set scrolling amount [0..GetScrollMaxX()]
IMGUI_API void SetScrollY(
    float scroll_y);  // set scrolling amount [0..GetScrollMaxY()]
...
IMGUI_API void SetScrollFromPosY(
    float pos_y,
    float center_y_ratio =
        0.5f);  // adjust scrolling amount to make given position valid. use
                // GetCursorPos() or GetCursorStartPos()+offset to get valid
                // positions.
...
IMGUI_API ImVec2
GetFontTexUvWhitePixel();  // get UV coordinate for a while pixel, useful to
...
IMGUI_API ImU32
GetColorU32(ImU32 col);  // retrieve given color with style alpha applied

// Parameters stacks (current window)
IMGUI_API void PushItemWidth(
    float
        item_width);  // width of items for the common item+label case, pixels.
                      // 0.0f = default to ~2/3 of windows width, >0.0f: width
                      // in pixels, <0.0f align xx pixels to the right of window
                      // (so -1.0f always align width to the right side)
IMGUI_API void PopItemWidth();
IMGUI_API float CalcItemWidth();  // width of item given pushed settings and
                                  // current cursor position
IMGUI_API void PushTextWrapPos(
    float wrap_pos_x =
        0.0f);  // word-wrapping for Text*() commands. < 0.0f: no wrapping;
                // 0.0f: wrap to end of window (or column); > 0.0f: wrap at
                // 'wrap_pos_x' position in window local space
IMGUI_API void PopTextWrapPos();
IMGUI_API void PushAllowKeyboardFocus(
    bool allow_keyboard_focus);  // allow focusing using TAB/Shift-TAB, enabled
                                 // by default but you can disable it for
                                 // certain widgets
...
IMGUI_API void SameLine(
    float pos_x = 0.0f,
    float spacing_w =
        -1.0f);  // call between widgets or groups to layout them horizontally
...
IMGUI_API void Unindent(
    float indent_w = 0.0f);  // move content position back to the left, by
                             // style.IndentSpacing or indent_w if != 0
IMGUI_API void
BeginGroup();  // lock horizontal starting position + capture group bounding box
               // into one "item" (so you can use IsItemHovered() or layout
               // primitives such as SameLine() on whole group, etc.)
IMGUI_API void EndGroup();
IMGUI_API ImVec2
GetCursorPos();  // cursor position is relative to window position
...
IMGUI_API ImVec2 GetCursorScreenPos();  // cursor position in absolute screen
                                        // coordinates [0..io.DisplaySize]
                                        // (useful to work with ImDrawList API)
IMGUI_API void SetCursorScreenPos(
    const ImVec2& screen_pos);  // cursor position in absolute screen
                                // coordinates [0..io.DisplaySize]
IMGUI_API void
AlignTextToFramePadding();  // vertically align upcoming text baseline to
                            // FramePadding.y so that it will align properly to
                            // regularly framed items (call if you have text on
                            // a line before a framed item)
IMGUI_API float GetTextLineHeight();  // ~ FontSize
IMGUI_API float
GetTextLineHeightWithSpacing();    // ~ FontSize + style.ItemSpacing.y (distance
...
// ID stack/scopes
// Read the FAQ for more details about how ID are handled in dear imgui. If you
// are creating widgets in a loop you most likely want to push a unique
// identifier (e.g. object pointer, loop index) to uniquely differentiate them.
// You can also use the "##foobar" syntax within widget label to distinguish
// them from each others. In this header file we use the "label"/"name"
// terminology to denote a string that will be displayed and used as an ID,
// whereas "str_id" denote a string that is only used as an ID and not aimed to
// be displayed.
IMGUI_API void PushID(
    const char* str_id);  // push identifier into the ID stack. IDs are hash of
                          // the entire stack!
...
IMGUI_API void Text(const char* fmt, ...)
    IM_FMTARGS(1);  // simple formatted text
IMGUI_API void TextV(const char* fmt, va_list args) IM_FMTLIST(1);
IMGUI_API void TextColored(const ImVec4& col, const char* fmt, ...)
    IM_FMTARGS(2);  // shortcut for PushStyleColor(ImGuiCol_Text, col);
...
IMGUI_API bool SmallButton(
    const char*
        label);  // button with FramePadding=(0,0) to easily embed within text
IMGUI_API bool InvisibleButton(
    const char* str_id,
    const ImVec2& size);  // button behavior without the visuals, useful to
                          // build custom behaviors using the public api (along
...
IMGUI_API bool ImageButton(
    ...
    const ImVec4& tint_col = ImVec4(1, 1, 1,
                                    1));  // <0 frame_padding uses default frame
...
IMGUI_API bool RadioButton(
    const char* label, bool active);  // use with e.g. if (RadioButton("one",
...
IMGUI_API void
Bullet();  // draw a small circle and keep the cursor on the same line. advance
           // cursor x position by GetTreeNodeToLabelSpacing(), same distance
           // that TreeNode() uses

// Widgets: Combo Box
// The new BeginCombo()/EndCombo() api allows you to manage your contents and
// selection state however you want it, by creating e.g. Selectable() items. The
// old Combo() api are helpers over BeginCombo()/EndCombo() which are kept
// available for convenience purpose.
...
// manually input values aren't clamped, can go off-bounds) For all the
// Float2/Float3/Float4/Int2/Int3/Int4 versions of every functions, note that a
// 'float v[X]' function argument is the same as 'float* v', the array syntax is
// just a way to document the number of elements that are expected to be
// accessible. You can pass address of your first element out of a contiguous
// set, e.g. &myvector.x Adjust format string to decorate the value with a
// prefix, a suffix, or adapt the editing and display precision e.g. "%.3f"
// -> 1.234; "%5.2f secs" -> 01.23 secs; "Biscuit: %.0f" -> Biscuit: 1; etc.
// Speed are per-pixel of mouse movement (v_speed=0.2f: mouse needs to move by 5
// pixels to increase value by 1). For gamepad/keyboard navigation, minimum
...
IMGUI_API bool DragFloatRange2(const char* label, float* v_current_min,
                               ...
                               float power = 1.0f);
IMGUI_API bool DragInt(
    const char* label, int* v, float v_speed = 1.0f, int v_min = 0,
    int v_max = 0,
...
IMGUI_API bool DragIntRange2(const char* label, int* v_current_min,
                             ...
                             const char* format = "%d",
                             const char* format_max = NULL);
IMGUI_API bool DragScalar(const char* label, ImGuiDataType data_type, void* v,
                          float v_speed, const void* v_min = NULL,
                          const void* v_max = NULL, const char* format = NULL,
                          float power = 1.0f);
IMGUI_API bool DragScalarN(const char* label, ImGuiDataType data_type, void* v,
                           int components, float v_speed,
                           const void* v_min = NULL, const void* v_max = NULL,
                           const char* format = NULL, float power = 1.0f);

// Widgets: Sliders (tip: ctrl+click on a slider to input with keyboard.
// manually input values aren't clamped, can go off-bounds) Adjust format string
// to decorate the value with a prefix, a suffix, or adapt the editing and
// display precision e.g. "%.3f" -> 1.234; "%5.2f secs" -> 01.23 secs; "Biscuit:
// %.0f" -> Biscuit: 1; etc.
IMGUI_API bool SliderFloat(
    ...
    float power = 1.0f);  // adjust format to decorate the value with a prefix
                          ...
                          // Use power!=1.0 for power curve sliders
...
IMGUI_API bool SliderInt4(const char* label, int v[4], int v_min, int v_max,
                          const char* format = "%d");
IMGUI_API bool SliderScalar(const char* label, ImGuiDataType data_type, void* v,
                            const void* v_min, const void* v_max,
                            const char* format = NULL, float power = 1.0f);
IMGUI_API bool SliderScalarN(const char* label, ImGuiDataType data_type,
                             void* v, int components, const void* v_min,
                             const void* v_max, const char* format = NULL,
                             float power = 1.0f);
...
// If you want to use InputText() with a dynamic string type such as std::string
// or your own, see misc/stl/imgui_stl.h
...
IMGUI_API bool InputInt4(const char* label, int v[4],
                         ImGuiInputTextFlags extra_flags = 0);
IMGUI_API bool InputDouble(const char* label, double* v, double step = 0.0f,
                           double step_fast = 0.0f, const char* format = "%.6f",
                           ImGuiInputTextFlags extra_flags = 0);
IMGUI_API bool InputScalar(const char* label, ImGuiDataType data_type, void* v,
                           const void* step = NULL,
                           const void* step_fast = NULL,
                           const char* format = NULL,
                           ImGuiInputTextFlags extra_flags = 0);
IMGUI_API bool InputScalarN(const char* label, ImGuiDataType data_type, void* v,
                            int components, const void* step = NULL,
                            const void* step_fast = NULL,
                            const char* format = NULL,
                            ImGuiInputTextFlags extra_flags = 0);

// Widgets: Color Editor/Picker (tip: the ColorEdit* functions have a little
// colored preview square that can be left-clicked to open a picker, and
// right-clicked to open an option menu.) Note that a 'float v[X]' function
// argument is the same as 'float* v', the array syntax is just a way to
// document the number of elements that are expected to be accessible. You can
// the pass the address of a first float element out of a contiguous structure,
// e.g. &myvector.x
...
IMGUI_API bool ColorButton(
    ...
    ImVec2 size = ImVec2(0, 0));  // display a colored square/button, hover for
                                  // details, return true when pressed.
IMGUI_API void SetColorEditOptions(
    ImGuiColorEditFlags
        flags);  // initialize current options (generally on application
                 // startup) if you want to select a default format, picker
                 // type, etc. User will be able to change many settings, unless
...
IMGUI_API bool TreeNode(const char* str_id, const char* fmt, ...) IM_FMTARGS(
    2);  // helper variation to completely decorelate the id from the displayed
...
IMGUI_API void TreeAdvanceToLabelPos();  // advance cursor x position by
...
GetTreeNodeToLabelSpacing();  // horizontal distance preceding label when using
                              ...
                              // TreeNode
IMGUI_API void SetNextTreeNodeOpen(
    bool is_open,
...
IMGUI_API bool CollapsingHeader(
    ...
    ImGuiTreeNodeFlags flags =
        0);  // when 'p_open' isn't NULL, display an additional small close
...
IMGUI_API bool ListBoxHeader(
    const char* label,
    const ImVec2& size = ImVec2(
        0, 0));  // use if you want to reimplement ListBox() will custom data or
                 // interactions. if the function return true, you can output
                 // elements then call ListBoxFooter() afterwards.
IMGUI_API bool ListBoxHeader(const char* label, int items_count,
                             int height_in_items = -1);  // "
IMGUI_API void
ListBoxFooter();  // terminate the scrolling region. only call ListBoxFooter()
                  // if ListBoxHeader() returned true!
...
IMGUI_API void PlotHistogram(const char* label, const float* values,
                             ...
                             ImVec2 graph_size = ImVec2(0, 0),
                             int stride = sizeof(float));
IMGUI_API void PlotHistogram(
    const char* label, float (*values_getter)(void* data, int idx), void* data,
    int values_count, int values_offset = 0, const char* overlay_text = NULL,
    float scale_min = FLT_MAX, float scale_max = FLT_MAX,
    ImVec2 graph_size = ImVec2(0, 0));

// Widgets: Value() Helpers. Output single value in "name: value" format (tip:
// freely declare more in your code to handle your types. you can add functions
...
IMGUI_API bool BeginMenu(
    const char* label,
    bool enabled = true);  // create a sub-menu entry. only call EndMenu() if
                           // this returns true!
IMGUI_API void EndMenu();  // only call EndMenu() if BeginMenu() returns true!
IMGUI_API bool MenuItem(
    const char* label, const char* shortcut = NULL, bool selected = false,
    bool enabled =
        true);  // return true when activated. shortcuts are displayed for
...
// Tooltips
IMGUI_API void
BeginTooltip();  // begin/append a tooltip window. to create full-featured
...
IMGUI_API void OpenPopup(
    const char*
        str_id);  // call to mark popup as open (don't call every frame!).
                  // popups are closed when user click outside, or if
                  // CloseCurrentPopup() is called within a
                  // BeginPopup()/EndPopup() block. By default,
                  // Selectable()/MenuItem() are calling CloseCurrentPopup().
                  // Popup identifiers are relative to the current ID-stack (so
                  // OpenPopup and BeginPopup needs to be at the same level).
IMGUI_API bool BeginPopup(
    const char* str_id,
    ImGuiWindowFlags flags =
        0);  // return true if the popup is open, and you can start outputting
             // to it. only call EndPopup() if BeginPopup() returns true!
IMGUI_API bool BeginPopupContextItem(
    const char* str_id = NULL,
    int mouse_button =
        1);  // helper to open and begin popup when clicked on last item. if you
             // can pass a NULL str_id only if the previous item had an id. If
             // you want to use that on a non-interactive item such as Text()
             // you need to pass in an explicit ID here. read comments in .cpp!
IMGUI_API bool BeginPopupContextWindow(
    const char* str_id = NULL, int mouse_button = 1,
    bool also_over_items = true);  // helper to open and begin popup when
                                   // clicked on current window.
IMGUI_API bool BeginPopupContextVoid(
    const char* str_id = NULL,
    int mouse_button = 1);  // helper to open and begin popup when clicked in
                            // void (where there are no imgui windows).
IMGUI_API bool BeginPopupModal(
    const char* name, bool* p_open = NULL,
    ImGuiWindowFlags flags =
        0);  // modal dialog (regular window with title bar, block interactions
             // behind the modal window, can't close the modal window by
             // clicking outside)
IMGUI_API void
EndPopup();  // only call EndPopup() if BeginPopupXXX() returns true!
IMGUI_API bool OpenPopupOnItemClick(
    const char* str_id = NULL,
    int mouse_button = 1);  // helper to open popup when clicked on last item.
                            // return true when just opened.
IMGUI_API bool IsPopupOpen(
    const char* str_id);             // return true if the popup is open
IMGUI_API void CloseCurrentPopup();  // close the popup we have begin-ed into.
                                     // clicking on a MenuItem or Selectable
                                     // automatically close the current popup.

// Columns
// You can also use SameLine(pos_x) for simplified columns. The columns API is
// still work-in-progress and rather lacking.
IMGUI_API void Columns(int count = 1, const char* id = NULL,
                       bool border = true);
IMGUI_API void NextColumn();     // next column, defaults to current row or next
                                 // row if the current row is finished
...
IMGUI_API void SetColumnWidth(int column_index,
                              float width);  // set column width (in pixels).
...
IMGUI_API void SetColumnOffset(
    int column_index,
    float offset_x);  // set position of column line (in pixels, from the left
                      // side of the contents region). pass -1 to use current
                      // column
IMGUI_API int GetColumnsCount();

// Logging/Capture: all text output from interface is captured to
// tty/file/clipboard. By default, tree nodes are automatically opened during
// logging.
IMGUI_API void LogToTTY(int max_depth = -1);  // start logging to tty
IMGUI_API void LogToFile(int max_depth = -1,
                         const char* filename = NULL);  // start logging to file
IMGUI_API void LogToClipboard(
    int max_depth = -1);     // start logging to OS clipboard
...
IMGUI_API bool BeginDragDropSource(
    ImGuiDragDropFlags flags =
        0);  // call when the current item is active. If this return true, you
...
IMGUI_API bool
BeginDragDropTarget();  // call after submitting an item that may receive an
                        // item. If this returns true, you can call
...
// SetScrollHere()" when applicable, to make your code more forward compatible
// when navigation branch is merged)
IMGUI_API void
...
IMGUI_API void SetKeyboardFocusHere(
    int offset = 0);  // focus keyboard on the next widget. Use positive
                      // 'offset' to access sub components of a multiple
                      // component widget. Use -1 to access previous widget.

// Utilities
// See Demo Window under "Widgets->Querying Status" for an interactive
// visualization of many of those functions.
...
IMGUI_API bool IsItemClicked(
    int mouse_button =
        0);  // is the last item clicked? (e.g. button/node just clicked on) ==
...
IMGUI_API bool
IsItemDeactivatedAfterEdit();  // was the last item just made inactive and made
                               // a value change when it was active? (e.g.
                               ...
                               // Combo()/ListBox()/Selectable() will return
                               // true even when clicking an already selected
                               // item).
IMGUI_API bool IsAnyItemHovered();
IMGUI_API bool IsAnyItemActive();
IMGUI_API bool IsAnyItemFocused();
IMGUI_API ImVec2
GetItemRectMin();  // get bounding rectangle of last item, in screen space
IMGUI_API ImVec2 GetItemRectMax();   // "
IMGUI_API ImVec2 GetItemRectSize();  // get size of last item, in screen space
IMGUI_API void
SetItemAllowOverlap();  // allow last item to be overlapped by a subsequent
                        // item. sometimes useful with invisible buttons,
...
IMGUI_API ImDrawList*
GetOverlayDrawList();  // this draw list will be the last rendered one, useful
                       // to quickly draw overlays shapes/text
...
IMGUI_API void SetStateStorage(
    ImGuiStorage* storage);  // replace current window storage with our own (if
                             // you want to manipulate it yourself, typically
                             // clear subsection of it)
...
IMGUI_API void CalcListClipping(
    ...
    int* out_items_display_end);  // calculate coarse clipping for large list of
                                  // evenly sized items. Prefer using the
                                  // ImGuiListClipper higher-level helper if you
                                  // can.

IMGUI_API bool BeginChildFrame(
    ImGuiID id, const ImVec2& size,
    ImGuiWindowFlags flags =
        0);  // helper to create a child window / scrolling region that looks
...
IMGUI_API int GetKeyIndex(
    ImGuiKey imgui_key);  // map ImGuiKey_* values into user's key index. ==
                          // io.KeyMap[key]
IMGUI_API bool IsKeyDown(
    int user_key_index);  // is key being held. == io.KeysDown[user_key_index].
                          // note that imgui doesn't know the semantic of each
                          // entry of io.KeysDown[]. Use your own indices/enums
                          // according to how your backend/engine stored them
                          // into io.KeysDown[]!
...
IMGUI_API bool IsMouseDown(
    int button);  // is mouse button held (0=left, 1=right, 2=middle)
...
IMGUI_API bool IsMouseDoubleClicked(
    int button);  // did mouse button double-clicked. a double-click returns
                  // false in IsMouseClicked(). uses io.MouseDoubleClickTime.
IMGUI_API bool IsMouseReleased(
    int button);  // did mouse button released (went from Down to !Down)
...
IMGUI_API ImVec2
GetMousePosOnOpeningCurrentPopup();  // retrieve backup of mouse position at the
                                     // time of opening popup we have
                                     // BeginPopup() into
IMGUI_API ImVec2 GetMouseDragDelta(
    int button = 0,
    float lock_threshold =
        -1.0f);  // dragging amount since clicking. if lock_threshold < -1.0f
                 // uses io.MouseDraggingThreshold
IMGUI_API void ResetMouseDragDelta(int button = 0);  //
IMGUI_API ImGuiMouseCursor
GetMouseCursor();  // get desired cursor type, reset in ImGui::NewFrame(), this
                   // is updated during the frame. valid before Render(). If you
                   // use software rendering by setting io.MouseDrawCursor ImGui
                   // will render those for you
IMGUI_API void SetMouseCursor(
    ImGuiMouseCursor type);  // set desired cursor type
IMGUI_API void CaptureKeyboardFromApp(
    bool capture = true);  // manually override io.WantCaptureKeyboard flag next
                           // frame (said flag is entirely left for your
                           // application to handle). e.g. force capture
                           // keyboard when your widget is being hovered.
IMGUI_API void CaptureMouseFromApp(
    bool capture =
        true);  // manually override io.WantCaptureMouse flag next frame (said
                // flag is entirely left for your application to handle).

// Clipboard Utilities (also see the LogToClipboard() function to capture or
...
// (default is "imgui.ini"). Set io.IniFilename to NULL to load/save manually.
// Read io.WantSaveIniSettings description about handling .ini saving manually.
...
IMGUI_API const char* SaveIniSettingsToMemory(
    size_t* out_ini_size =
        NULL);  // return a zero-terminated string with the .ini data which you
                ...
                // is set, then save data by your own mean and clear
                // io.WantSaveIniSettings.

// Memory Utilities
// All those functions are not reliant on the current context.
// If you reload the contents of imgui.cpp at runtime, you may need to call
// SetCurrentContext() + SetAllocatorFunctions() again.
...
enum ImGuiWindowFlags_ {
  ...
  ImGuiWindowFlags_NoScrollbar = 1
                                 << 3,  // Disable scrollbars (window can still
                                        // scroll with mouse or programatically)
  ...
  ImGuiWindowFlags_NoInputs = 1
                              << 9,  // Disable catching mouse or keyboard
  ...
  ImGuiWindowFlags_NoFocusOnAppearing =
      1 << 12,  // Disable taking focus when transitioning from hidden to
                // visible state
  ...
  ImGuiWindowFlags_NavFlattened =
      1 << 23,  // [BETA] Allow gamepad/keyboard navigation to cross over parent
                // border to this child (only use on child that have no
                // scrolling!)
...
enum ImGuiInputTextFlags_ {
  ...
  ImGuiInputTextFlags_CharsHexadecimal = 1
                                         << 1,  // Allow 0123456789ABCDEFabcdef
  ImGuiInputTextFlags_CharsUppercase = 1 << 2,  // Turn a..z into A..Z
  ImGuiInputTextFlags_CharsNoBlank = 1 << 3,    // Filter out spaces, tabs
  ImGuiInputTextFlags_AutoSelectAll =
      1 << 4,  // Select entire text when first taking mouse focus
  ImGuiInputTextFlags_EnterReturnsTrue =
      1 << 5,  // Return 'true' when Enter is pressed (as opposed to when the
               // value was modified)
  ImGuiInputTextFlags_CallbackCompletion =
      1 << 6,  // Call user function on pressing TAB (for completion handling)
  ImGuiInputTextFlags_CallbackHistory =
      1 << 7,  // Call user function on pressing Up/Down arrows (for history
               // handling)
  ImGuiInputTextFlags_CallbackAlways =
      1 << 8,  // Call user function every time. User code may query cursor
               // position, modify text buffer.
  ImGuiInputTextFlags_CallbackCharFilter =
      1 << 9,  // Call user function to filter character. Modify data->EventChar
               // to replace/filter input, or return 1 in callback to discard
               // character.
...
enum ImGuiTreeNodeFlags_ {
  ...
  // ImGuITreeNodeFlags_SpanAllAvailWidth  = 1 << 11,  // FIXME: TODO: Extend
  // hit box horizontally even if not framed ImGuiTreeNodeFlags_NoScrollOnOpen =
...
enum ImGuiSelectableFlags_ {
  ...
  ImGuiSelectableFlags_SpanAllColumns =
      1 << 1,  // Selectable frame can span all columns (text will still fit in
               // current column)
  ImGuiSelectableFlags_AllowDoubleClick =
      1 << 2,  // Generate press events on double clicks too
  ImGuiSelectableFlags_Disabled =
      1 << 3  // Cannot be selected, display greyed out text
};
...
enum ImGuiComboFlags_ {
  ...
  ImGuiComboFlags_HeightSmall =
      1 << 1,  // Max ~4 items visible. Tip: If you want your combo popup to be
               // a specific size you can use SetNextWindowSizeConstraints()
...
enum ImGuiFocusedFlags_ {
  ...
  ImGuiFocusedFlags_ChildWindows =
      1 << 0,  // IsWindowFocused(): Return true if any children of the window
               // is focused
  ImGuiFocusedFlags_RootWindow =
      1 << 1,  // IsWindowFocused(): Test from root window (top most parent of
...
// imgui or to your app, you should use the 'io.WantCaptureMouse' boolean for
// that. Please read the FAQ! Note: windows with the ImGuiWindowFlags_NoInputs
...
enum ImGuiHoveredFlags_ {
  ...
  ImGuiHoveredFlags_AllowWhenBlockedByPopup =
      1 << 3,  // Return true even if a popup window is normally blocking access
               // to this item/window
  // ImGuiHoveredFlags_AllowWhenBlockedByModal     = 1 << 4,   // Return true
  ...
  ImGuiHoveredFlags_AllowWhenDisabled =
      1 << 7,  // Return true even if the item is disabled
...
enum ImGuiDragDropFlags_ {
  ...
  ImGuiDragDropFlags_SourceNoPreviewTooltip =
      1 << 0,  // By default, a successful call to BeginDragDropSource opens a
               // tooltip so you can display a preview or description of the
               // source contents. This flag disable this behavior.
  ImGuiDragDropFlags_SourceNoDisableHover =
      1 << 1,  // By default, when dragging we clear data so that
               // IsItemHovered() will return false, to avoid subsequent user
               // code submitting tooltips. This flag disable this behavior so
               // you can still call IsItemHovered() on the source item.
  ...
  ImGuiDragDropFlags_SourceExtern =
      1 << 4,  // External source (from outside of imgui), won't attempt to read
               // current item/window info. Will always return true. Only one
               // Extern source can be active simultaneously.
  ImGuiDragDropFlags_SourceAutoExpirePayload =
      1 << 5,  // Automatically expire the payload if the source cease to be
               // submitted (otherwise payloads are persisting while being
               // dragged)
  ...
  ImGuiDragDropFlags_AcceptNoDrawDefaultRect =
      1 << 11,  // Do not draw the default highlight rectangle when hovering
                // over target.
...
// User fill ImGuiIO.KeyMap[] array with indices into the ImGuiIO.KeysDown[512]
// array
enum ImGuiKey_ {
  ImGuiKey_Tab,
  ImGuiKey_LeftArrow,
...
// NewFrame() will automatically fill io.NavInputs[] based on your io.KeysDown[]
// + io.KeyMap[] arrays. Gamepad:  Set io.ConfigFlags |=
// ImGuiConfigFlags_NavEnableGamepad to enable. Back-end: set
// ImGuiBackendFlags_HasGamepad and fill the io.NavInputs[] fields before
// calling NewFrame(). Note that io.NavInputs[] is cleared by EndFrame(). Read
// instructions in imgui.cpp for more details. Download PNG/PSD at
// http://goo.gl/9LgVZW.
enum ImGuiNavInput_ {
  // Gamepad Mapping
  ImGuiNavInput_Activate,  // activate / open / toggle / tweak value       //
                           // e.g. Cross  (PS4), A (Xbox), A (Switch), Space
                           // (Keyboard)
  ImGuiNavInput_Cancel,  // cancel / close / exit                        // e.g.
                         // Circle (PS4), B (Xbox), B (Switch), Escape
                         // (Keyboard)
  ImGuiNavInput_Input,   // text input / on-screen keyboard              // e.g.
                        // Triang.(PS4), Y (Xbox), X (Switch), Return (Keyboard)
  ImGuiNavInput_Menu,  // tap: toggle menu / hold: focus, move, resize // e.g.
                       // Square (PS4), X (Xbox), Y (Switch), Alt (Keyboard)
  ImGuiNavInput_DpadLeft,     // move / tweak / resize window (w/ PadMenu)    //
                              // e.g. D-pad Left/Right/Up/Down (Gamepads), Arrow
                              // keys (Keyboard)
  ImGuiNavInput_DpadRight,    //
  ImGuiNavInput_DpadUp,       //
  ImGuiNavInput_DpadDown,     //
  ImGuiNavInput_LStickLeft,   // scroll / move window (w/ PadMenu)            //
                              // e.g. Left Analog Stick Left/Right/Up/Down
  ImGuiNavInput_LStickRight,  //
  ImGuiNavInput_LStickUp,     //
  ImGuiNavInput_LStickDown,   //
  ImGuiNavInput_FocusPrev,    // next window (w/ PadMenu)                     //
                              // e.g. L1 or L2 (PS4), LB or LT (Xbox), L or ZL
                              // (Switch)
  ImGuiNavInput_FocusNext,    // prev window (w/ PadMenu)                     //
                              // e.g. R1 or R2 (PS4), RB or RT (Xbox), R or ZL
                              // (Switch)
  ImGuiNavInput_TweakSlow,    // slower tweaks                                //
                              // e.g. L1 or L2 (PS4), LB or LT (Xbox), L or ZL
                              // (Switch)
  ImGuiNavInput_TweakFast,    // faster tweaks                                //
                              // e.g. R1 or R2 (PS4), RB or RT (Xbox), R or ZL
                              // (Switch)

  // [Internal] Don't use directly! This is used internally to differentiate
  // keyboard from gamepad inputs for behaviors that require to differentiate
  // them. Keyboard behavior that have no corresponding gamepad mapping (e.g.
  // CTRL+TAB) will be directly reading from io.KeysDown[] instead of
  // io.NavInputs[].
...
enum ImGuiConfigFlags_ {
  ...
  ImGuiConfigFlags_NavEnableGamepad =
      1 << 1,  // Master gamepad navigation enable flag. This is mostly to
               // instruct your imgui back-end to fill io.NavInputs[]. Back-end
               // also needs to set ImGuiBackendFlags_HasGamepad.
  ImGuiConfigFlags_NavEnableSetMousePos =
      1 << 2,  // Instruct navigation to move the mouse cursor. May be useful on
               ...
               // binding, otherwise ImGui will react as if the mouse is jumping
  ...
  ImGuiConfigFlags_NoMouse =
      1
      << 4,  // Instruct imgui to clear mouse position/buttons in NewFrame().
             // This allows ignoring the mouse information set by the back-end.
  ImGuiConfigFlags_NoMouseCursorChange =
      1 << 5,  // Instruct back-end to not alter mouse cursor shape and
               // visibility. Use if the back-end cursor changes are interfering
               // with yours and you don't want to use SetMouseCursor() to
               // change mouse cursor. You may want to honor requests from imgui
               // by reading GetMouseCursor() yourself instead.

  // User storage (to allow your back-end/engine to communicate to code that may
...
// Back-end capabilities flags stored in io.BackendFlags. Set by imgui_impl_xxx
// or custom back-end.
enum ImGuiBackendFlags_ {
  ImGuiBackendFlags_HasGamepad =
      1 << 0,  // Back-end supports gamepad and currently has one connected.
  ImGuiBackendFlags_HasMouseCursors =
      1 << 1,  // Back-end supports honoring GetMouseCursor() value to change
               // the OS cursor shape.
  ImGuiBackendFlags_HasSetMousePos =
      1 << 2  // Back-end supports io.WantSetMousePos requests to reposition the
              // OS mouse position (only used if
...
// ImGuiStyle structure. NB: the enum only refers to fields of ImGuiStyle which
// makes sense to be pushed/popped inside UI code. During initialization, feel
// free to just poke into ImGuiStyle directly. NB: if changing this enum, you
// need to update the associated internal table GStyleVarInfo[] accordingly.
...
enum ImGuiColorEditFlags_ {
  ...
  ImGuiColorEditFlags_NoAlpha =
      1 << 1,  //              // ColorEdit, ColorPicker, ColorButton: ignore
               //              Alpha component (read 3 components from the input
               //              pointer).
  ...
  ImGuiColorEditFlags_NoLabel =
      1 << 7,  //              // ColorEdit, ColorPicker: disable display of
               //              inline text label (the label is still forwarded
  ...
  ImGuiColorEditFlags_RGB =
      1 << 20,  // [Inputs]     // ColorEdit: choose one among RGB/HSV/HEX.
  ...
  ImGuiColorEditFlags_Uint8 =
      1 << 23,  // [DataType]   // ColorEdit, ColorPicker, ColorButton:
  ...
  ImGuiColorEditFlags_PickerHueBar =
      1 << 25,  // [PickerMode] // ColorPicker: bar for Hue, rectangle for
                // Sat/Value.
  ImGuiColorEditFlags_PickerHueWheel =
      1 << 26,  // [PickerMode] // ColorPicker: wheel for Hue, triangle for
                // Sat/Value.

  // [Internal] Masks
  ImGuiColorEditFlags__InputsMask = ImGuiColorEditFlags_RGB |
  ...
  ImGuiColorEditFlags__OptionsDefault =
      ...
      ImGuiColorEditFlags_PickerHueBar  // Change application default using
                                        // SetColorEditOptions()
};

// Enumeration for GetMouseCursor()
// User code may request binding to display given cursor by calling
...
enum ImGuiMouseCursor_ {
  ...
  ImGuiMouseCursor_ResizeEW,    // When hovering over a vertical border or a
                                // column
  ImGuiMouseCursor_ResizeNESW,  // When hovering over the bottom-left corner of
                                // a window
  ImGuiMouseCursor_ResizeNWSE,  // When hovering over the bottom-right corner of
                                // a window
  ImGuiMouseCursor_Hand,        // (Unused by imgui functions. Use for e.g.
                                // hyperlinks)
...
// Condition for ImGui::SetWindow***(), SetNextWindow***(), SetNextTreeNode***()
// functions Important: Treat as a regular enum! Do NOT combine multiple values
// using binary operators! All the functions above treat 0 as a shortcut to
// ImGuiCond_Always.
enum ImGuiCond_ {
  ...
  ImGuiCond_FirstUseEver =
      1 << 2,  // Set the variable if the object/window has no persistently
...
struct ImGuiStyle {
  ...
  ImVec2 WindowMinSize;    // Minimum window size. This is a global setting. If
  ...
  ImVec2 ButtonTextAlign;  // Alignment of button text when button is larger
                           // than text. Defaults to (0.5f,0.5f) for
                           // horizontally+vertically centered.
  ImVec2 DisplayWindowPadding;    // Window position are clamped to be visible
...
// ImGui::GetIO(). Read 'Programmer guide' section in .cpp file for general
// usage.
struct ImGuiIO {
  //------------------------------------------------------------------
  ...
  ImGuiConfigFlags
      ConfigFlags;  // = 0                  // See ImGuiConfigFlags_ enum. Set
                    // by user/application. Gamepad/keyboard navigation options,
                    // etc.
  ImGuiBackendFlags
      BackendFlags;  // = 0                  // Set ImGuiBackendFlags_ enum. Set
                     // by imgui_impl_xxx files or custom back-end to
                     // communicate features supported by the back-end.
  ImVec2 DisplaySize;  // <unset>              // Main display size, in pixels.
                       // For clamping windows positions.
  ...
  const char* IniFilename;  // = "imgui.ini"        // Path to .ini file. Set
                            // NULL to disable automatic .ini loading/saving, if
                            // e.g. you want to manually load/save from memory.
  const char*
      LogFilename;  // = "imgui_log.txt"    // Path to .log file (default
                    // parameter to ImGui::LogToFile when no file is specified).
  float MouseDoubleClickTime;     // = 0.30f              // Time for a
                                  // double-click, in seconds.
  float MouseDoubleClickMaxDist;  // = 6.0f               // Distance threshold
                                  // to stay in to validate a double-click, in
                                  // pixels.
  float MouseDragThreshold;       // = 6.0f               // Distance threshold
                                  // before considering we are dragging.
  int KeyMap[ImGuiKey_COUNT];     // <unset>              // Map of indices into
                                  // the KeysDown[512] entries array which
                                  // represent your "native" keyboard state.
  float KeyRepeatDelay;  // = 0.250f             // When holding a key/button,
                         // time before it starts repeating, in seconds (for
                         // buttons in Repeat mode, etc.).
  float KeyRepeatRate;   // = 0.050f             // When holding a key/button,
                         // rate at which it repeats, in seconds.
  void* UserData;  // = NULL               // Store your own data for retrieval
                   // by callbacks.

  ImFontAtlas*
      Fonts;  // <auto>               // Load and assemble one or more fonts
              // into a single tightly packed texture. Output to Fonts array.
  float FontGlobalScale;      // = 1.0f               // Global scale all fonts
  bool FontAllowUserScaling;  // = false              // Allow user scaling text
                              // of individual window with CTRL+Wheel.
  ImFont* FontDefault;  // = NULL               // Font to use on NewFrame().
                        // Use NULL to uses Fonts->Fonts[0].
  ImVec2
      DisplayFramebufferScale;  // = (1.0f,1.0f)        // For retina display or
                                // other situations where window coordinates are
                                // different from framebuffer coordinates. User
                                // storage only, presently not used by ImGui.
  ImVec2 DisplayVisibleMin;     // <unset> (0.0f,0.0f)  // [obsolete] If you use
                             // DisplaySize as a virtual space larger than your
                             // screen, set DisplayVisibleMin/Max to the visible
                             // area.
  ImVec2 DisplayVisibleMax;  // <unset> (0.0f,0.0f)  // [obsolete: just use
                             // io.DisplaySize] If the values are the same, we
                             // defaults to Min=(0.0f) and Max=DisplaySize
  ...
  bool ConfigMacOSXBehaviors;  // = defined(__APPLE__) // OS X style: Text
                               // editing cursor movement using Alt instead of
                               // Ctrl, Shortcuts using Cmd/Super instead of
                               // Ctrl, Line/Text Start and End using Cmd+Arrows
                               // instead of Home/End, Double click selects by
                               // word instead of selecting whole text,
                               // Multi-selection in lists uses Cmd/Super
                               // instead of Ctrl (was called
                               // io.OptMacOSXBehaviors prior to 1.63)
  bool ConfigInputTextCursorBlink;  // = true           // Set to false to
                                    // disable blinking cursor, for users who
                                    // consider it distracting. (was called:
                                    // io.OptCursorBlink prior to 1.63)
  bool
      ConfigResizeWindowsFromEdges;  // = false          // [BETA] Enable
                                     // resizing of windows from their edges and
                                     // from the lower-left corner. This
  ...
  // cursor for text input position (e.g. when using Japanese/Chinese IME in
  // Windows) (default to use native imm32 api on Windows)
  void (*ImeSetInputScreenPosFn)(int x, int y);
  void* ImeWindowHandle;  // (Windows) Set this to your HWND to get automatic
                          // IME cursor positioning.

#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
  // [OBSOLETE since 1.60+] Rendering function, will be automatically called in
  // Render(). Please call your rendering function yourself now! You can obtain
  // the ImDrawData* by calling ImGui::GetDrawData() after Render(). See example
  // applications if you are unsure of how to implement this.
  void (*RenderDrawListsFn)(ImDrawData* data);
#else
  // This is only here to keep ImGuiIO the same size, so that
  // IMGUI_DISABLE_OBSOLETE_FUNCTIONS can exceptionally be used outside of
  // imconfig.h.
  ...
  bool
      MouseDown[5];  // Mouse buttons: 0=left, 1=right, 2=middle + extras. ImGui
                     // itself mostly only uses left button (BeginPopupContext**
                     // are using right button). Others buttons allows us to
  ...
  ImWchar InputCharacters[16 + 1];  // List of characters input (translated by
                                    // user from keypress+keyboard state). Fill
                                    // using AddInputCharacter() helper.
  float NavInputs[ImGuiNavInput_COUNT];  // Gamepad inputs (keyboard keys will
                                         // be auto-mapped and be written here
                                         // by ImGui::NewFrame, all values will
                                         // be cleared back to zero in
                                         // ImGui::EndFrame)

  // Functions
  IMGUI_API void AddInputCharacter(
      ImWchar c);  // Add new character into InputCharacters[]
  ...
  bool WantCaptureMouse;  // When io.WantCaptureMouse is true, imgui will use
                          // the mouse inputs, do not dispatch them to your main
                          // game/application (in both cases, always pass on
                          // mouse inputs to imgui). (e.g. unclicked mouse is
                          // hovering over an imgui window, widget is active,
                          // mouse was clicked over an imgui window, etc.).
  bool WantCaptureKeyboard;  // When io.WantCaptureKeyboard is true, imgui will
                             // use the keyboard inputs, do not dispatch them to
                             // your main game/application (in both cases,
                             // always pass keyboard inputs to imgui). (e.g.
                             // InputText active, or an imgui window is focused
                             // and navigation is enabled, etc.).
  bool WantTextInput;  // Mobile/console: when io.WantTextInput is true, you may
                       // display an on-screen keyboard. This is set by ImGui
                       // when it wants textual keyboard input to happen (e.g.
                       // when a InputText widget is active).
  bool WantSetMousePos;      // MousePos has been altered, back-end should
                             // reposition mouse on next frame. Set only when
                             // ImGuiConfigFlags_NavEnableSetMousePos flag is
                             // enabled.
  bool WantSaveIniSettings;  // When manual .ini load/save is active
                             ...
                             // IMPORTANT: You need to clear
                             // io.WantSaveIniSettings yourself.
  bool NavActive;   // Directional navigation is currently allowed (will handle
                    // ImGuiKey_NavXXX events) = a window is focused and it
                    // doesn't use the ImGuiWindowFlags_NoNavInputs flag.
  bool NavVisible;  // Directional navigation is visible and allowed (will
                    // handle ImGuiKey_NavXXX events).
  float Framerate;  // Application framerate estimation, in frame per second.
                    // Solely for convenience. Rolling average estimation based
  ...
  int MetricsActiveAllocations;  // Number of active allocations, updated by
                                 // MemAlloc/MemFree based on current context.
                                 // May be off if you have multiple imgui
                                 // contexts.
  ImVec2
      MouseDelta;  // Mouse delta. Note that this is zero if either current or
                   // previous position are invalid (-FLT_MAX,-FLT_MAX), so a
                   // disappearing/reappearing mouse won't have a huge delta.

  //------------------------------------------------------------------
  // [Internal] ImGui will maintain those fields. Forward compatibility not
  // guaranteed!
  //------------------------------------------------------------------

  ImVec2 MousePosPrev;  // Previous mouse position temporary storage (nb: not
                        // for public use, set to MousePos in NewFrame())
  ...
  bool MouseDownOwned[5];  // Track if button was clicked inside a window. We
                           // don't request mouse capture from the application
                           // if click started outside ImGui bounds.
  ...
  ImVec2 MouseDragMaxDistanceAbs[5];  // Maximum distance, absolute, on each
  ...
  float KeysDownDuration[512];  // Duration the keyboard key has been down (0.0f
                                // == just pressed)
...
//-----------------------------------------------------------------------------
// Obsolete functions (Will be removed! Read 'API BREAKING CHANGES' section in
...
IMGUI_API bool InputFloat(const char* label, float* v, float step,
                          float step_fast, int decimal_precision,
                          ImGuiInputTextFlags extra_flags =
                              0);  // Use the 'const char* format' version
                                   // instead of 'decimal_precision'!
IMGUI_API bool InputFloat2(const char* label, float v[2], int decimal_precision,
                           ImGuiInputTextFlags extra_flags = 0);
IMGUI_API bool InputFloat3(const char* label, float v[3], int decimal_precision,
                           ImGuiInputTextFlags extra_flags = 0);
IMGUI_API bool InputFloat4(const char* label, float v[4], int decimal_precision,
                           ImGuiInputTextFlags extra_flags = 0);
...
IMGUI_API bool Begin(
    const char* name, bool* p_open, const ImVec2& size_on_first_use,
...
// (also: Windows implementation of STL with debug enabled is absurdly slow, so
// let's bypass it so our code runs fast in debug). *Important* Our
// implementation does NOT call C++ constructors/destructors. This is
// intentional, we do not require it but you have to be mindful of that. Do
// _not_ use this class as a std::vector replacement in your code!
template <typename T>
class ImVector {
 public:
  int Size;
  ...
  inline void reserve(int new_capacity) {
    ...
    value_type* new_data =
        (value_type*)ImGui::MemAlloc((size_t)new_capacity * sizeof(value_type));
    if (Data) {
  ...
  // NB: It is forbidden to call push_back/push_front/insert with a reference
  ...
  inline iterator erase(const_iterator it) {
    ...
    memmove(Data + off, Data + off + 1,
            ((size_t)Size - (size_t)off - 1) * sizeof(value_type));
    Size--;
    return Data + off;
  }
  inline iterator erase(const_iterator it, const_iterator it_last) {
    ...
    memmove(Data + off, Data + off + count,
            ((size_t)Size - (size_t)off - count) * sizeof(value_type));
    Size -= (int)count;
    return Data + off;
  }
  ...
  inline iterator insert(const_iterator it, const value_type& v) {
    ...
    if (off < (int)Size)
      memmove(Data + off + 1, Data + off,
              ((size_t)Size - (size_t)off) * sizeof(value_type));
...
// Helper: IM_NEW(), IM_PLACEMENT_NEW(), IM_DELETE() macros to call MemAlloc +
// Placement New, Placement Delete + MemFree We call C++ constructor on own
// allocated memory via the placement "new(ptr) Type()" syntax. Defining a
...
// want to quickly create an UI within deep-nested code that runs multiple times
...
// statement so make sure you don't use it within e.g. an if() statement without
// curly braces.
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS  // Will obsolete
#define IMGUI_ONCE_UPON_A_FRAME         \
  static ImGuiOnceUponAFrame imgui_oaf; \
  if (imgui_oaf)
#endif
...
struct ImGuiStorage {
  ...
  // Use on your own storage if you know only integer are being stored
...
// return 0 by default. Special processing:
// - ImGuiInputTextFlags_CallbackCharFilter:  return 1 if the character is not
// allowed. You may also set 'EventChar=0' as any character replacement are
// allowed.
// - ImGuiInputTextFlags_CallbackResize:      notified by InputText() when the
// string is resized. BufTextLen is set to the new desired string length so you
// can update the string size on your side of the fence. You can also replace
// Buf pointer if your underlying data is reallocated. No need to initialize new
// characters or zero-terminator as InputText will do it right after the resize
// callback.
struct ImGuiInputTextCallbackData {
  ...
  ImWchar EventChar;  // Character input                      // Read-write   //
                      // [CharFilter] Replace character or set to zero. return 1
                      // is equivalent to setting EventChar=0;
  ImGuiKey EventKey;  // Key pressed (Up/Down/TAB)            // Read-only    //
                      // [Completion,History]
  char* Buf;          // Text buffer                          // Read-write   //
              // [Resize] Can replace pointer / [Completion,History,Always] Only
              // write to pointed data, don't replace the actual pointer!
...
typedef ImGuiInputTextCallback
    ImGuiTextEditCallback;  // [OBSOLETE 1.63+] Made the names consistent
...
struct ImGuiPayload {
  ...
  char DataType[32 + 1];   // Data type tag (short user-supplied string, 32
                           // characters max)
  bool Preview;   // Set when AcceptDragDropPayload() was called and mouse has
                  // been hovering the target item (nb: handle overlapping drag
                  // targets)
  bool Delivery;  // Set when AcceptDragDropPayload() was called and mouse
...
#define IM_COL32_WHITE \
  IM_COL32(255, 255, 255, 255)                 // Opaque white = 0xFFFFFFFF
#define IM_COL32_BLACK IM_COL32(0, 0, 0, 255)  // Opaque black
#define IM_COL32_BLACK_TRANS \
  IM_COL32(0, 0, 0, 0)  // Transparent black = 0x00000000

// Helper: ImColor() implicity converts colors to either ImU32 (packed 4x1 byte)
// or ImVec4 (4x1 float) Prefer using IM_COL32() macros if you want a guaranteed
// compile-time ImU32 for usage with ImDrawList API.
...
// If you are submitting lots of evenly spaced items and you have a random
// access to the list, you can perform coarse clipping based on visibility to
// save yourself from processing those items at all. The clipper calculates the
// range of visible items and advance the cursor to compensate for the
// non-visible items we have skipped. ImGui already clip items based on their
// bounds but it needs to measure text size to do so. Coarse clipping before
...
// - Step 0: the clipper let you process the first element, regardless of it
// being visible or not, so we can measure the element height (step skipped if
// we passed a known height as second arg to constructor).
// - Step 1: the clipper infer height from first element, calculate the actual
// range of elements to display, and position the cursor before the first
// element.
// - (Step 2: dummy step only required if an explicit items_height was passed to
// constructor or Begin() and user call Step(). Does nothing and switch to
// Step 3.)
// - Step 3: the clipper validate that we have reached the expected Y position
...
struct ImGuiListClipper {
  ...
  // items_count:  Use -1 to ignore (you can call Begin later). Use INT_MAX if
  // you don't know how many items you have (in which case the cursor won't be
  // advanced in the final step). items_height: Use -1.0f to be calculated
  // automatically on first step. Otherwise pass in the distance between your
  // items, typically GetTextLineHeightWithSpacing() or
  // GetFrameHeightWithSpacing(). If you don't specify an items_height, you NEED
  // to call Step(). If you specify items_height you may call the old
  ...
  IMGUI_API void Begin(
      int items_count,
      float items_height =
          -1.0f);  // Automatically called by constructor if you passed
...
struct ImDrawCmd {
  unsigned int
      ElemCount;    // Number of indices (multiple of 3) to be rendered as
                    // triangles. Vertices are stored in the callee ImDrawList's
                    // vtx_buffer[] array, indices in idx_buffer[].
  ImVec4 ClipRect;  // Clipping rectangle (x1, y1, x2, y2). Subtract
  ...
  ImDrawCallback
      UserCallback;  // If != NULL, call the function instead of rendering the
...
// Vertex index (override with '#define ImDrawIdx unsigned int' inside in
// imconfig.h)
#ifndef ImDrawIdx
...
// struct or use a typedef) NOTE: IMGUI DOESN'T CLEAR THE STRUCTURE AND DOESN'T
// CALL A CONSTRUCTOR SO ANY CUSTOM FIELD WILL BE UNINITIALIZED. IF YOU ADD
// EXTRA FIELDS (SUCH AS A 'Z' COORDINATES) YOU WILL NEED TO CLEAR THEM DURING
// RENDER OR TO IGNORE THEM.
...
enum ImDrawCornerFlags_ {
  ...
  ImDrawCornerFlags_Top =
      ImDrawCornerFlags_TopLeft | ImDrawCornerFlags_TopRight,  // 0x3
  ImDrawCornerFlags_Bot =
      ImDrawCornerFlags_BotLeft | ImDrawCornerFlags_BotRight,  // 0xC
  ImDrawCornerFlags_Left =
      ImDrawCornerFlags_TopLeft | ImDrawCornerFlags_BotLeft,  // 0x5
  ImDrawCornerFlags_Right =
      ImDrawCornerFlags_TopRight | ImDrawCornerFlags_BotRight,  // 0xA
  ImDrawCornerFlags_All =
      0xF  // In your function calls you may use ~0 (= all bits sets) instead of
           // ImDrawCornerFlags_All, as a convenience
};
...
// ImGuiIO::RenderDrawListFn function for rendering. Each ImGui window contains
// its own ImDrawList. You can use ImGui::GetWindowDrawList() to access the
// current window draw list and draw custom primitives. You can interleave
// normal ImGui:: calls and adding primitives to the current draw list. All
// positions are generally in pixel coordinates (top-left at (0,0), bottom-right
// at io.DisplaySize), but you are totally free to apply whatever transformation
// matrix to want to the data (if you apply such transformation you'll want to
// apply it to ClipRect as well) Important: Primitives are always added to the
// list and not culled (culling is done at higher-level by ImGui:: functions),
// if you use this API a lot consider coarse culling your drawn objects.
struct ImDrawList {
  // This is what you have to render
  ImVector<ImDrawCmd>
      CmdBuffer;  // Draw commands. Typically 1 command = 1 GPU draw call,
  ...
  const ImDrawListSharedData* _Data;  // Pointer to shared draw data (you can
                                      // use ImGui::GetDrawListSharedData() to
  ...
  ImVector<ImDrawChannel>
      _Channels;  // [Internal] draw channels for columns API (not resized down
                  // so _ChannelsCount may be smaller than _Channels.Size)
  ...
  IMGUI_API void PushClipRect(
      ImVec2 clip_rect_min, ImVec2 clip_rect_max,
      bool intersect_with_current_clip_rect =
          false);  // Render-level scissoring. This is passed down to your
                   // render function but not used for CPU-side coarse clipping.
                   // Prefer using higher-level ImGui::PushClipRect() to affect
  ...
  IMGUI_API void AddRect(
      const ImVec2& a, const ImVec2& b, ImU32 col, float rounding = 0.0f,
      int rounding_corners_flags = ImDrawCornerFlags_All,
      float thickness =
          1.0f);  // a: upper-left, b: lower-right, rounding_corners_flags:
  ...
  IMGUI_API void AddRectFilledMultiColor(const ImVec2& a, const ImVec2& b,
                                         ImU32 col_upr_left,
                                         ImU32 col_upr_right,
                                         ImU32 col_bot_right,
                                         ImU32 col_bot_left);
  IMGUI_API void AddQuad(const ImVec2& a, const ImVec2& b, const ImVec2& c,
                         const ImVec2& d, ImU32 col, float thickness = 1.0f);
  IMGUI_API void AddQuadFilled(const ImVec2& a, const ImVec2& b,
                               const ImVec2& c, const ImVec2& d, ImU32 col);
  IMGUI_API void AddTriangle(const ImVec2& a, const ImVec2& b, const ImVec2& c,
                             ImU32 col, float thickness = 1.0f);
  IMGUI_API void AddTriangleFilled(const ImVec2& a, const ImVec2& b,
                                   const ImVec2& c, ImU32 col);
  IMGUI_API void AddCircle(const ImVec2& centre, float radius, ImU32 col,
                           int num_segments = 12, float thickness = 1.0f);
  IMGUI_API void AddCircleFilled(const ImVec2& centre, float radius, ImU32 col,
                                 int num_segments = 12);
  ...
  IMGUI_API void AddImageQuad(ImTextureID user_texture_id, const ImVec2& a,
                              ...
                              ImU32 col = 0xFFFFFFFF);
  IMGUI_API void AddImageRounded(ImTextureID user_texture_id, const ImVec2& a,
                                 const ImVec2& b, const ImVec2& uv_a,
                                 const ImVec2& uv_b, ImU32 col, float rounding,
                                 int rounding_corners = ImDrawCornerFlags_All);
  IMGUI_API void AddPolyline(const ImVec2* points, const int num_points,
                             ImU32 col, bool closed, float thickness);
  IMGUI_API void AddConvexPolyFilled(
      const ImVec2* points, const int num_points,
      ImU32 col);  // Note: Anti-aliased filling requires points to be in
                   // clockwise order.
  IMGUI_API void AddBezierCurve(const ImVec2& pos0, const ImVec2& cp0,
  ...
  IMGUI_API void PathArcTo(const ImVec2& centre, float radius, float a_min,
                           float a_max, int num_segments = 10);
  IMGUI_API void PathArcToFast(
      const ImVec2& centre, float radius, int a_min_of_12,
      int a_max_of_12);  // Use precomputed angles for a 12 steps circle
  IMGUI_API void PathBezierCurveTo(const ImVec2& p1, const ImVec2& p2,
                                   const ImVec2& p3, int num_segments = 0);
  IMGUI_API void PathRect(const ImVec2& rect_min, const ImVec2& rect_max,
                          float rounding = 0.0f,
                          int rounding_corners_flags = ImDrawCornerFlags_All);

  // Channels
  // - Use to simulate layers. By switching channels to can render out-of-order
  // (e.g. submit foreground primitives before background primitives)
  // - Use to minimize draw calls (e.g. if going back-and-forth between multiple
  // non-overlapping clipping rectangles, prefer to append into separate
  // channels then merge at the end)
  ...
  AddDrawCmd();  // This is useful if you need to forcefully create a new draw
                 // call (to allow for dependent rendering / blending).
                 // Otherwise primitives are merged into the same draw-call as
                 // much as possible
...
// (NB: the style and the naming convention here is a little inconsistent but we
// preserve them for backward compatibility purpose)
struct ImDrawData {
  bool Valid;  // Only valid after Render() is called and before the next
               // NewFrame() is called.
  ImDrawList**
      CmdLists;  // Array of ImDrawList* to render. The ImDrawList are owned by
                 // ImGuiContext and only pointed to from here.
  int CmdListsCount;  // Number of ImDrawList* to render
  int TotalIdxCount;  // For convenience, sum of all ImDrawList's IdxBuffer.Size
  int TotalVtxCount;  // For convenience, sum of all ImDrawList's VtxBuffer.Size
  ImVec2 DisplayPos;  // Upper-left position of the viewport to render (==
                      // upper-left of the orthogonal projection matrix to use)
  ImVec2
      DisplaySize;  // Size of the viewport to render (== io.DisplaySize for the
                    // main viewport) (DisplayPos + DisplaySize == lower-right
  ...
  IMGUI_API void ScaleClipRects(
      const ImVec2&
          sc);  // Helper to scale the ClipRect field of each ImDrawCmd. Use if
                // your final output buffer is at a different scale than ImGui
                // expects, or if there is a difference between your window
...
struct ImFontConfig {
  ...
  const ImWchar*
      GlyphRanges;  // NULL     // Pointer to a user-provided list of Unicode
                    // range (2 value per range, values are inclusive,
                    // zero-terminated list). THE ARRAY DATA NEEDS TO PERSIST AS
                    // LONG AS THE FONT IS ALIVE.
  float GlyphMinAdvanceX;  // 0        // Minimum AdvanceX for glyphs, set Min
                           // to align font icons, set both Min/Max to enforce
                           // mono-space font
  ...
  bool MergeMode;  // false    // Merge into previous ImFont, so you can combine
                   ...
                   // icons + Japanese glyphs). You may want to use
                   // GlyphOffset.y when merge font of different heights.
  unsigned int RasterizerFlags;  // 0x00     // Settings for custom font
                                 // rasterizer (e.g. ImGuiFreeType). Leave as
                                 // zero if you aren't using one.
...
enum ImFontAtlasFlags_ {
  ...
  ImFontAtlasFlags_NoMouseCursors =
      1 << 1  // Don't build software mouse cursors into the atlas
};
...
// just as well.
// - This is an old API and it is currently awkward for those and and various
// other reasons! We will address them in the future!
struct ImFontAtlas {
  ...
  // are UTF-8 and NOT in your local code page. In C++11, you can create UTF-8
  // string literal using the u8"Hello world" syntax. See FAQ for details. NB:
  // Consider using GlyphRangesBuilder to build glyph ranges from textual data.
  ...
  // Helpers to build glyph ranges from text data. Feed your application
  // strings/characters to it then call BuildRanges().
  ...
  // own purposes. After calling Build(), you can query the rectangle position
  // and render your pixels. You can also request your rectangles to be mapped
  // as font glyph (given a font + Unicode point), so you can render e.g. custom
  // colorful icons and use them as regular glyphs.
  struct CustomRect {
    unsigned int
        ID;  // Input    // User ID. Use <0x10000 to map into a font glyph,
             // >=0x10000 for other/internal/custom texture data.
    unsigned short Width, Height;  // Input    // Desired rectangle dimension
    unsigned short X, Y;           // Output   // Packed position in Atlas
    float GlyphAdvanceX;           // Input    // For custom font glyphs only
                                   // (ID<0x10000): glyph xadvance
    ImVec2 GlyphOffset;            // Input    // For custom font glyphs only
  ...
  IMGUI_API int AddCustomRectRegular(
      unsigned int id, int width,
      int height);  // Id needs to be >= 0x10000. Id >= 0x80000000 are reserved
                    // for ImGui and ImDrawList
  IMGUI_API int AddCustomRectFontGlyph(
      ImFont* font, ImWchar id, int width, int height, float advance_x,
      const ImVec2& offset =
          ImVec2(0, 0));  // Id needs to be < 0x10000 to register a rectangle to
                          // map into a specific font.
  ...
  // [Internal]
  IMGUI_API void CalcCustomRectUV(const CustomRect* rect, ImVec2* out_uv_min,
                                  ImVec2* out_uv_max);
  ...
  bool Locked;  // Marked as Locked by ImGui::NewFrame() so attempt to modify
                // the atlas will assert.
  ImFontAtlasFlags Flags;  // Build flags (see ImFontAtlasFlags_)
  ImTextureID TexID;       // User data to refer to the texture once it has been
                      // uploaded to user's graphic systems. It is passed back
                      // to you during rendering via the ImDrawCmd structure.
  ...
  ImVector<ImFont*>
      Fonts;  // Hold all the fonts returned by AddFont*. Fonts[0] is the
              // default font upon calling ImGui::NewFrame(), use
              // ImGui::PushFont()/PopFont() to change the current font.
  ImVector<CustomRect> CustomRects;   // Rectangles for packing custom texture
                                      // data into the atlas.
  ImVector<ImFontConfig> ConfigData;  // Internal data
  int CustomRectIds[1];  // Identifiers of custom texture rectangle used by
                         // ImFontAtlas/ImDrawList
};
...
struct ImFont {
  ...
  float FontSize;  // <user set>   // Height of characters, set during loading
                   // (don't change after loading)
  float Scale;  // = 1.f        // Base font scale, multiplied by the per-window
                // font scale which you can adjust with SetFontScale()
  ImVec2 DisplayOffset;  // = (0.f,0.f)  // Offset font rendering by xx pixels
  ImVector<ImFontGlyph> Glyphs;  //              // All glyphs.
  ImVector<float>
      IndexAdvanceX;  //              // Sparse. Glyphs->AdvanceX in a directly
                      //              indexable way (more cache-friendly, for
                      //              CalcTextSize functions which are often
                      //              bottleneck in large UI).
  ImVector<ImWchar> IndexLookup;     //              // Sparse. Index glyphs by
                                     //              Unicode code-point.
  const ImFontGlyph* FallbackGlyph;  // == FindGlyph(FontFallbackChar)
  float FallbackAdvanceX;            // == FallbackGlyph->AdvanceX
  ImWchar FallbackChar;  // = '?'        // Replacement glyph if one isn't
                         // found. Only set via SetFallbackChar()

  // Members: Cold ~18/26 bytes
  short ConfigDataCount;  // ~ 1          // Number of ImFontConfig involved in
                          // creating this font. Bigger than 1 when merging
                          // multiple font sources into one ImFont.
  ImFontConfig*
      ConfigData;  //              // Pointer within ContainerAtlas->ConfigData
  ImFontAtlas* ContainerAtlas;  //              // What we has been loaded into
  float Ascent, Descent;  //              // Ascent: distance from top to bottom
                          //              of e.g. 'A' [0..FontSize]
  bool DirtyLookupTables;
  int MetricsTotalSurface;  //              // Total surface in pixels to get an
                            //              idea of the font
  ...
  IMGUI_API void AddGlyph(ImWchar c, float x0, float y0, float x1, float y1,
                          float u0, float v0, float u1, float v1,
                          float advance_x);
  IMGUI_API void AddRemapChar(
      ImWchar dst, ImWchar src,
      bool overwrite_dst = true);  // Makes 'dst' character/glyph points to
                                   // 'src' character/glyph. Currently needs to
